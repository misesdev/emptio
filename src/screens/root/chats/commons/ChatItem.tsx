...
import useChatStore, { ChatUser } from "@services/zustand/useChatStore"
import { useAccount } from "@src/context/AccountContext"
import { useService } from "@src/providers/ServiceProvider"
import { Utilities } from "@src/utils/Utilities"
import { User } from "@services/user/types/User"
import { FilterChat } from "./ChatList"
...
const ChatItem = ({ item, filters,
    ...
    const loadItemData = useCallback(async () => {
        ...
        if(!profile) 
            profile = await userService.getUser(item.lastMessage.pubkey)

        if(eventRef.current.content.includes("?iv=")) {
            // const decrypted = await messageService.decryptMessage(user, item.lastMessage)
            await eventRef.current.decrypt() //.content = decrypted.content
        }
        ...
        if(!filters.find(f => f.chat_id == item.chat_id)) {
            filters.push({
                ...
                user_name: Utilities.getUserName(profile, 30),
    ...
    return (
        <Animated.View style={[styles.chatContainer, { backgroundColor }]}>
            ...
            >
                <TouchableOpacity onPress={() => showProfile(follow??{} as User)} style={{ width: "15%" }}>
                    <ProfilePicture user={follow??{} as User} size={50} />
                </TouchableOpacity>
                <View style={{ width: "60%", overflow: "hidden" }}>
                    ...
                    <View style={{ width: "100%", paddingHorizontal: 6 }}>
                        <Text style={styles.message}>
                            {Utilities.getClipedContent(eventRef.current.content.replaceAll("\n", " "), 26)}
                        </Text>
                    </View>
                </View>                    
